...
| 2 | **Single-wall 20 mm cube** (spiral vase), at the #1 temperature | `extrusion_multiplier` | calipers on the wall at 8 points against the 0.45 mm extrusion width | **Printed 2026-09-21**, 16 min, clean completion. **The one-line TPU wall went floppy about a third of the way up and was dragged into loops**; the lower wall was measured. **8 readings 0.40–0.42 mm against 0.45 → `extrusion_multiplier` 1.10** (TPU seats deeper in the drive gear and feeds short). Actual flow is now ~8 % above the tower's, inside 210 °C's 20 °C margin, so the 2.0 cap stands. Checked by the coupon arms. **Released easily, light pressure only, at `z_offset` −0.02** (owner) — offset unchanged pending the coupon's thin arms. Next TPU cube: `--height 8` |
