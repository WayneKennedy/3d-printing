...
| 2026-09-21 16:08 | `tpu_flow_cube.gcode` | 16 min | **Completed; suspect.** Spiral-vase single wall, `tpu` at 210 °C. Wall intact about a third of the way up, then loops of strands — first read as a floppy wall, but see the next job: feeding may already have been failing. Lower wall 0.40–0.42 mm (8 readings) against 0.45. Released easily. [open-questions.md](open-questions.md#tpu-95a--profile-and-calibration) |
| 2026-09-21 17:31 | `tpu_deck_coupon.gcode` | 72.9 min of 125 est. | **Failed: no filament extruded at any point** — no purge line, no skirt, nothing on the bed (owner). Ran on air until the owner noticed; stopped by `M112` at Z 5.4, nozzle holding 210 °C. Telemetry could not show it: `filament_used` counts commanded extrusion. Cause under investigation. |
