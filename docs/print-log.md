...
| 2026-09-09 16:37 | `wrp_flip_organic.gcode` | 255.3 min (4 h 15) | **Complete; part not yet inspected.** SO-ARM101 `Wrist_Roll_Pitch`, flipped, third attempt: **organic support from the bed only**, sliced by the new **PrusaSlicer 2.9.6** (first print from it) with `plaplus_soarm` + `--support-material-style organic`. White PLA+. 255.3 min vs 261.3 est (−2.3 %); **12.69 m = 30.5 cm³ = 37.9 g against a sliced 37.8 g**. Support reached the fork face from the bed with nothing standing on the part (verified in G-code before starting). Ran unmonitored by choice, 12:17–16:37. **To report: did the interface under the fork face release cleanly, and did the trees mark the outer faces.** Those two answers also decide whether 2.9.6 is validated. |
