...
### USB over-current dropped the MCU mid-print (2026-09-21)
...
**Cause: unknown.** All ports at once points at one event on the shared USB supply rather than
one device drawing too much — and two low-draw devices are nowhere near the enabled 1.6 A
budget. Candidates, none verified: something plugged, unplugged or knocked at that moment (ask
first); a brief short or ESD on a cable; a transient from the printer back through its USB
cable. **If it recurs**, the timestamp against what was happening at the machine is the evidence
that narrows it. A self-powered hub between Pi and printer is the standard isolation, and is
already listed as unverified-but-known-to-matter under [The USB constraint](#the-usb-constraint--verified-on-printhub-2026-09-07).
